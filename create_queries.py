...
def write_links(index_f, dataset, ent_out, small_ent_out, max_ans_num, name):
    ...
    with open('./data/%s/%s/%s-queries.pkl'%(dataset, index_f, name), 'wb') as f:
        pickle.dump(queries, f)
    with open('./data/%s/%s/%s-easy-answers.pkl'%(dataset, index_f, name), 'wb') as f:
        pickle.dump(tp_answers, f)
    with open('./data/%s/%s/%s-hard-answers.pkl'%(dataset, index_f, name), 'wb') as f:
        pickle.dump(fn_answers, f)
    with open('./data/%s/%s/%s-answers.pkl'%(dataset, index_f, name), 'wb') as f:
        pickle.dump(fp_answers, f)
    print (num_more_answer)

def ground_queries(index_f, dataset, query_structure, ent_in, ent_out, small_ent_in, small_ent_out, gen_num, max_ans_num, query_name, mode, ent2id, rel2id):
    ...
    name_to_save = '%s'%(mode)
    with open('./data/%s/%s/%s-queries.pkl'%(dataset, index_f, name_to_save), 'wb') as f:
        pickle.dump(queries, f)
    with open('./data/%s/%s/%s-answers.pkl'%(dataset, index_f, name_to_save), 'wb') as f:
        pickle.dump(fp_answers, f)
    with open('./data/%s/%s/%s-hard-answers.pkl'%(dataset, index_f, name_to_save), 'wb') as f:
        pickle.dump(fn_answers, f)
    with open('./data/%s/%s/%s-easy-answers.pkl'%(dataset,index_f, name_to_save), 'wb') as f:
        pickle.dump(tp_answers, f)
    return queries, tp_answers, fp_answers, fn_answers

def generate_queries(index_f, dataset, query_structures, gen_num, max_ans_num, gen_train, gen_valid, gen_test, query_names, save_name):
    ...
    if query_structure == ['e', ['r']]:
        if gen_train:
            write_links(index_f, dataset, train_ent_out, defaultdict(lambda: defaultdict(set)), max_ans_num, 'train')
        if gen_valid:
            write_links(index_f, dataset, valid_only_ent_out, train_ent_out, max_ans_num, 'valid')
        if gen_test:
            write_links(index_f, dataset, test_only_ent_out, valid_ent_out, max_ans_num, 'test')
    ...
    s0 = time.time()
    if gen_train:
        train_queries, train_tp_answers, train_fp_answers, train_fn_answers = ground_queries(index_f, dataset, query_structure,
    ...
    if gen_valid:
        valid_queries, valid_tp_answers, valid_fp_answers, valid_fn_answers = ground_queries(index_f, dataset, query_structure,
            valid_ent_in, valid_ent_out, train_ent_in, train_ent_out, gen_num[1], max_ans_num, query_name, 'valid', ent2id, rel2id)
    if gen_test:
        test_queries, test_tp_answers, test_fp_answers, test_fn_answers = ground_queries(index_f, dataset, query_structure,
...
@click.command()
@click.option('--dataset', default="Yelp")
...
def main(dataset, seed, gen_train_num, gen_valid_num, gen_test_num, max_ans_num, reindex, gen_train, gen_valid, gen_test, gen_id, save_name, index_only):
    train_num_dict = {'FB15k': 273710, "FB15k-237": 149689, "NELL": 107982,"Yelp": 15000}
    valid_num_dict = {'FB15k': 8000, "FB15k-237": 5000, "NELL": 4000, "Yelp": 4000}
    test_num_dict = {'FB15k': 8000, "FB15k-237": 5000, "NELL": 4000, "Yelp": 4000}
    gen_train = True
    gen_valid = True
    gen_test = True
    ...
    #gen_id = 0
    #generate_queries(0,dataset, query_structures[gen_id:gen_id+1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[gen_id:gen_id+1], save_name)
    gen_id = 1
    generate_queries(1,dataset, query_structures[gen_id:gen_id+1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[gen_id:gen_id+1], save_name)
    gen_id = 2
    generate_queries(2,dataset, query_structures[gen_id:gen_id+1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[gen_id:gen_id+1], save_name)
    gen_id = 3
    generate_queries(3,dataset, query_structures[gen_id:gen_id+1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[gen_id:gen_id+1], save_name)
    gen_id = 4
    generate_queries(4,dataset, query_structures[gen_id:gen_id+1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[gen_id:gen_id+1], save_name)
    gen_id = 5
    generate_queries(5,dataset, query_structures[gen_id:gen_id+1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[gen_id:gen_id+1], save_name)
    gen_id = 6
    generate_queries(6,dataset, query_structures[gen_id:gen_id+1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[gen_id:gen_id+1], save_name)
    gen_id = 12
    generate_queries(12,dataset, query_structures[gen_id:gen_id+1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[gen_id:gen_id+1], save_name)
    gen_id = 13
    generate_queries(13,dataset, query_structures[gen_id:gen_id+1], [gen_train_num, gen_valid_num, gen_test_num], max_ans_num, gen_train, gen_valid, gen_test, query_names[gen_id:gen_id+1], save_name)
